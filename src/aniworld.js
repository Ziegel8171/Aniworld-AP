...
// Enhanced with persistent language memory and in-progress episode marking
...
(function() {
  ...
  // ============================================================
  // Yellow Marking for In-Progress Episodes (started but not finished)
  // ============================================================
  function addInProgressEpisodeStyles() {
    const style = document.createElement('style');
    style.textContent = `
      /* Yellow background for episodes that have been started but not finished */
      a[data-episode-id].aniskip-in-progress {
        background: #ffdd00 !important;
        color: #000 !important;
      }
    `;
    document.head.appendChild(style);
    console.log('VOE AniSkip: Added in-progress episode styles');
  }

  // Mark episodes that have saved playback positions
  async function markInProgressEpisodes() {
    try {
      // Get all saved playback positions from storage
      const allStorage = await browser.storage.local.get(null);
      
      // Find all playback position keys
      const inProgressEpisodes = {};
      for (const key of Object.keys(allStorage)) {
        if (key.startsWith('playbackPosition_')) {
          // Extract episode info from key: playbackPosition_animename_s1_e5
          const match = key.match(/playbackPosition_(.+)_s(\d+)_e(\d+)$/);
          if (match) {
            const season = parseInt(match[2], 10);
            const episode = parseInt(match[3], 10);
            inProgressEpisodes[`s${season}e${episode}`] = true;
          }
        }
      }
      
      console.log('VOE AniSkip: In-progress episodes:', inProgressEpisodes);
      
      // Get current page's season from URL
      const seasonMatch = window.location.pathname.match(/\/staffel-(\d+)/i);
      const currentSeason = seasonMatch ? parseInt(seasonMatch[1], 10) : 1;
      
      // Find all episode links and mark the ones that are in progress
      ...
      episodeLinks.forEach(link => {
        ...
        if (epMatch) {
          const episodeNum = parseInt(epMatch[1], 10);
          const key = `s${currentSeason}e${episodeNum}`;
          
          // Check if this episode is in progress (has saved position)
          if (inProgressEpisodes[key]) {
            // Don't mark if it's the currently active episode
            if (!link.classList.contains('active')) {
              link.classList.add('aniskip-in-progress');
              console.log('VOE AniSkip: Marked episode', episodeNum, 'as in-progress');
            }
          }
        }
      });
    } catch (e) {
      console.error('VOE AniSkip: Error marking in-progress episodes:', e);
    }
  }

  // Add styles immediately
  addInProgressEpisodeStyles();
